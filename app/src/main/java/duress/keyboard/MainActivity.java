...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		AutoRunSwitch.setText(
			isRussianDevice
			? "Атозапуск экрана с полем ввода после перезагрузки (для автозапуска клавиатуры, чтобы сразу начать реагировать на тригеры, включенныё здесь). Рекомендуется включить эту опцию. Может однократно блокировать экран после перезагрузки чтобы избежать ошибок наложения на экран блокировки. Это не проблема, ведь экран после перезагрузки и так заблокирован. И то что Actvity Exported тоже не является проблемой, ведь блокировка будет однократной при любом количестве запусков."
			: "Automatically launch the input field screen after reboot (to automatically launch the keyboard so it immediately begins responding to triggers enabled here). It is recommended to enable this option. May lock the screen once after reboot to avoid overlay errors on the lock screen. This is not a problem, as the screen is already locked after reboot. And the fact that the Activity is Exported is not an issue either, since the lock will be one-time, regardless of the number of launches."
		);
