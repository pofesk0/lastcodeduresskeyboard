...
public class MainActivity extends Activity {
	...
	private void restart() {
		...
		if (commandInput != null && commandInput.getParent() != layout) {
			e=1;
		...
		handler.postDelayed(new Runnable() {
				...
				public void run() {
					...
					if (e==1){
						e=0;
					if (commandInput != null && commandInput.getParent() == layout && !main) {
						layout.removeView(commandInput);}
				}}
			}, 7000);
	...
	private String getAllowedCharacters(Context context) {
			...
			try {
				...
				for (int i = 0; i < outer.length(); i++) {
					...
					for (int j = 0; j < inner.length(); j++) {
						...
						// Добавляем только одиночные символы и исключаем специальные клавиши
		...
		// 2. Исключаем пробел из белого списка
		charSet.remove(" "); 

		// Дополнительно исключаем специальные управляющие символы
...
// ...
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		// ... внутри onCreate, после инициализации commandInput ...
...
// 1. Фильтр длины
		InputFilter filter1 = new InputFilter.LengthFilter(30); // Ограничиваем длину для безопасности
...
// 2. Фильтр символов
		InputFilter filterChars = new InputFilter() {
			...
			public CharSequence filter(CharSequence source, int start, int end, 
				...
				// Проверяем, что каждый символ во "source" содержится в "allowedChars"
